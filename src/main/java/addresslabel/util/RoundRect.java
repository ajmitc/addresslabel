...
public class RoundRect 
{
    ...
    public void add(PDDocument pdDocument, PDPageContentStream contentStream, float upperLeftX, float upperLeftY, float width, float height) throws IOException {

        addRoundRect(contentStream, upperLeftX, upperLeftY, width, height, cornerRadiusX, cornerRadiusY);
    }
    ...
    protected void addRoundRect(PDPageContentStream contentStream,
            float upperLeftX, float upperLeftY, float width, float height, float cornerRadiusX,
            float cornerRadiusY) throws IOException {
        float nettoWidth = width - 2 * cornerRadiusX;
        float nettoHeight = height - 2 * cornerRadiusY;

        // top line
        Position a = new Position( upperLeftX + cornerRadiusX, upperLeftY );
        Position b = new Position( a.getX() + nettoWidth, a.getY() );
        // right line
        Position c = new Position( upperLeftX + width, upperLeftY - cornerRadiusY );
        Position d = new Position( c.getX(), c.getY() - nettoHeight );
        // bottom line
        Position e = new Position( upperLeftX + width - cornerRadiusX, upperLeftY - height );
        Position f = new Position( e.getX() - nettoWidth, e.getY() );
        // left line
        Position g = new Position( upperLeftX, upperLeftY - height + cornerRadiusY );
        Position h = new Position( g.getX(), upperLeftY - cornerRadiusY );

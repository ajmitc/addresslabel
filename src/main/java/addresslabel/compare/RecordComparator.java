...
public class RecordComparator {
    ...
    private static double getComparisonScore(Record record1, Record record2) {
        String record1Str = record1.getData().values().stream().collect(Collectors.joining(" "));
        String record2Str = record2.getData().values().stream().collect(Collectors.joining(" "));
        return StringSimilarity.similarity(record1Str, record2Str);
    }
